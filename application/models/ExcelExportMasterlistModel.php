...
LOWER(CONCAT(LEFT(s.first_name, 1), '.', s.last_name, RIGHT(s.LRN, 1))) as pass_word, 
...
$query = $this->db->query("SELECT mi.masterlist_id as masterlist, mi.extra_accounts as extra_accounts FROM `cbt_students` s LEFT JOIN cbt_masterlist_info mi ON (s.school_code = mi.school_code AND s.grade_level = mi.grade_level AND s.section_code = mi.section_code) LEFT JOIN cbt_add_custom_assessment ca ON (mi.assessment_id = ca.assessment_id) LEFT JOIN cbt_add_section ad ON (s.section_id = ad.section_id) WHERE mi.masterlist_id = '".$sid."' ORDER BY student_id;");
