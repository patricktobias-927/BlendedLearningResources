...
class Filter extends CI_Controller {
    ...
    public function load_filter()
    {
        ...
        else
        {
            ...
            // $this->db->join('cbt_students cs', 'cs.school_code = cas.school_code');
         $this->db->distinct();
            $this->db->select('cas.school_code, cas.grade, cas.section_name, cas.section_code, cas.school_year, (SELECT  COUNT(*) section_code FROM cbt_students WHERE section_code = cas.section_code ) as population');
            // $this->db->from('cbt_add_section cas');
